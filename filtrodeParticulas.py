...
def genera_filtro(num_particulas, balizas, real, centro=[2,2], radio=3):
  ...
  filtro = [] #lista de objetos de la clase robot.
  medidas = real.sense(balizas) #tomamos medidas del robot real para poder calcular el peso de las particulas.
  for i in range(num_particulas): #Añado al filtro el conjunto de partículas.
    filtro.append(real.copy())
    filtro[-1].set(centro[0]+random.uniform(-radio,radio),\
                   centro[1]+random.uniform(-radio,radio),\
                   random.uniform(-pi,pi))
    filtro[-1].measurement_prob(medidas,balizas) #acutalizo el peso de las partículas.
  return filtro
...
for punto in objetivos:
  while distancia(trayectoria[-1],punto) > EPSILON and len(trayectoria) <= 1000:
    ...
    if w > W:  w =  W
    if w < -W: w = -W
    v = distancia(trayectoria[-1],punto)
    if (v > V): v = V
    if (v < 0): v = 0
    if HOLONOMICO:
      if GIROPARADO and abs(w) > .01:
          v = 0 # Mover todas las partículas aquí
      real.move(w,v)
      measurement = real.sense(objetivos)
    ...
    #pesos nuevos del filtro y añadir la hipótesis a la trayectoria.
